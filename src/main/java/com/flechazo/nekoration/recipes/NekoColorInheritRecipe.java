...
// This type of shaped crafting recipe is used in Nekoration to transfer color information
// from ingrediants to result items, which is stored in their NBT.
public class NekoColorInheritRecipe extends CustomRecipe implements net.minecraftforge.common.crafting.IShapedRecipe<CraftingContainer> {
    // Half-Timber Ingrediants or Results...
    ...
    public ItemStack assemble(CraftingContainer inv, RegistryAccess access) {
        ...
        if (HALF_TIMBERS.test(finalResult))
            ...
            // Wooden Dyeable Block Color Inheritance
            ...
            // Dyeable Block Color Inheritance
    ...
    public static class Serializer implements RecipeSerializer<NekoColorInheritRecipe> {
        public NekoColorInheritRecipe fromJson(ResourceLocation id, JsonObject json) {
            ...
            String[] astring = NekoColorInheritRecipe
                    .shrink(NekoColorInheritRecipe.patternFromJson(GsonHelper.getAsJsonArray(json, "pattern")));
            int i = astring[0].length();
            int j = astring.length;
            NonNullList<Ingredient> nonnulllist = NekoColorInheritRecipe.dissolvePattern(astring, ingredientKeys, i, j);
            ItemStack itemstack = NekoColorInheritRecipe.itemFromJson(GsonHelper.getAsJsonObject(json, "result"));
            return new NekoColorInheritRecipe(id, s, i, j, nonnulllist, itemstack);
        }

        public NekoColorInheritRecipe fromNetwork(ResourceLocation id, FriendlyByteBuf packet) {
        ...
        public void toNetwork(FriendlyByteBuf packet, NekoColorInheritRecipe recipe) {
    ...
    // JEI Support
    private boolean dyeable(ItemStack stack) {
        return HALF_TIMBERS.test(stack) || WOODEN.test(stack) || DYEABLES.test(stack);
    }
    ...
    public List<List<ItemStack>> getInputs() {
        ...
        for (Ingredient ingredient : this.recipeItems) { // For each slot...
            ...
            for (ItemStack ingrediantStack : ingredient.getItems()) { // For each possible item in this slot...
                Item ingrediantItem = ingrediantStack.getItem();
                if (dyeable(this.result) && dyeable(ingrediantStack)) { // Color Inheritance...
                    if (ingrediantItem instanceof DyeableBlockItem)
                        for (EnumNekoColor color : EnumNekoColor.values()) {
                            ItemStack variantStack = new ItemStack(ingrediantItem, ingrediantStack.getCount());
                            DyeableBlockItem.setColor(variantStack, color);
                            variants.add(variantStack);
                        }
                    else if (ingrediantItem instanceof DyeableWoodenBlockItem)
                        for (EnumWoodenColor color : EnumWoodenColor.values()) {
                            ItemStack variantStack = new ItemStack(ingrediantItem, ingrediantStack.getCount());
                            DyeableWoodenBlockItem.setColor(variantStack, color);
                            variants.add(variantStack);
                        }
                    else if (ingrediantItem instanceof HalfTimberBlockItem)
                        for (EnumWoodenColor color : EnumWoodenColor.values()) {
                            ItemStack variantStack = new ItemStack(ingrediantItem, ingrediantStack.getCount());
                            HalfTimberBlockItem.setColor1(variantStack, EnumNekoColor.WHITE);
                            HalfTimberBlockItem.setColor0(variantStack, color);
                            variants.add(variantStack);
                        }
                    else variants.add(ingrediantStack.copy());
                } else {
    ...
    public List<ItemStack> getOutputs() {
        ...
        if (dyeable(this.result)) { // Dyeable but does not contain color nbt, iterate all variants...
            if (resultItem instanceof DyeableBlockItem)
                for (EnumNekoColor color : EnumNekoColor.values()) {
                    ItemStack variantStack = new ItemStack(resultItem, this.result.getCount());
                    DyeableBlockItem.setColor(variantStack, color);
                    variants.add(variantStack);
                }
            else if (resultItem instanceof DyeableWoodenBlockItem)
                for (EnumWoodenColor color : EnumWoodenColor.values()) {
                    ItemStack variantStack = new ItemStack(resultItem, this.result.getCount());
                    DyeableWoodenBlockItem.setColor(variantStack, color);
                    variants.add(variantStack);
                }
            else if (resultItem instanceof HalfTimberBlockItem)
                for (EnumWoodenColor color : EnumWoodenColor.values()) {
                    ItemStack variantStack = new ItemStack(resultItem, this.result.getCount());
                    HalfTimberBlockItem.setColor1(variantStack, EnumNekoColor.WHITE);
                    HalfTimberBlockItem.setColor0(variantStack, color);
            ...
            else variants.add(this.result.copy());
        } else {
